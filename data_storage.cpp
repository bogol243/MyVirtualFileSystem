...
void DataStorage::Seekg(File* fd, size_t offset) {
	fd->gpos = offset;
}

void DataStorage::Seekp(File* fd, size_t offset) {
	fd->ppos = offset;
}

void DataStorage::SeekgEnd(File* fd) {
	fd->gpos = fd->inode_obj.byte_size;
}

void DataStorage::SeekpEnd(File* fd) {
